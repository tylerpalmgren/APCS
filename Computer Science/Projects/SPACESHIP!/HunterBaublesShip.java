...
public class HunterBaublesShip implements Spaceship<TheHungerBaublesGameInfo> {
   ...
   public static void main(String[] args){
      TextClient.run("10.40.30.41", new HunterBaublesShip());
      //TextClient.run("10.1.31.86", new HunterBaublesShip());
   }
   ...
   public ShipCommand getNextCommand(Environment <TheHungerBaublesGameInfo> env){   
      ...
      System.out.println("High Score :" + env.getGameInfo().getBestScore());
      System.out.println("Score: " + (env.getGameInfo().getScore()-2));
      System.out.println("Energy: " + ship.getEnergy());
      
      if(env.getGameInfo().getBestScore() >= 15){
    ...
    //  System.out.println("Angle: " + ship.getPosition().getAngleTo(Solids.get().getPosition()));
      ...
      if(DANGER.size() > 0){            //currently moving at 11 units or more 
         ...
         if(DeathCone.size() != 0){
            ...
            if(ship.getSpeed() <= 90){
               return new ThrustCommand('B', 3, 1);
            }
            ...
            if(Baubles.size() > 0 && env.getGameInfo().getNumBaublesCarried() < 25 && full == false){ //currently moving at 11 units or more  pos = ship.getPosition();
               ...
               if(pos.getDistanceTo(baublePos) < (pos.getDistanceTo(closestObject.getPosition()) + 10) && ship.getSpeed() != 0){
                  return new SteerCommand(angleTo, false);
               }
            } 
            else if(env.getGameInfo().getBaublesCarriedWeight() == 25){
               full = true;
               pos = ship.getPosition();
               ObjectStatus bauble = Baubles.get(0);
               Point baublePos = Baubles.get(0).getPosition();
