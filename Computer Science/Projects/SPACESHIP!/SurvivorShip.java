...
public class SurvivorShip extends BasicSpaceship{
   ...
   public static void main(String[] args){
      TextClient.run("10.40.30.41", new SurvivorShip());
      //TextClient.run("10.1.27.13", new SurvivorShip());
   }
     ...
     // System.out.println("Speed: " + ship.getSpeed());
      ...
      if(DANGER.size() > 0){            //currently moving at 11 units or more 
         ...
         if(DeathCone.size() != 0){
            ...
            if((!(closestObject.getType().equals("Dragon")) && !(closestObject.getType().equals("BlackHole")) && !(closestObject.getType().equals("Star")) &&  !(closestObject.getType().equals("Nebula")) &&  !(closestObject.getType().equals("WormHole")) && !(closestObject.getType().equals("Asteroids"))) && pos.getDistanceTo(closestObject.getPosition()) <= 75 && ship.getSpeed() != 0){
         ...
         // might need to compensate to being right next to an object, as in a few pixels away
      ...
      if(ship.getSpeed() > 50 && emergency == false){
         return new BrakeCommand(0.8);
      }
      /*if(DANGER.size() > 0 && ship.getMovementDirection() == ship.getPosition().getAngleTo(DANGER.get(0).getPosition())){
      for(){
      if(ship.getMovementDirection() == ship.getPosition().getAngleTo(DANGER.get().getPosition())){
